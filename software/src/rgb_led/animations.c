...
uint8_t inverseBrightness = 0;
uint16_t phases[NUM_LEDS];
...
uint32_t GetLEDRainbow(uint32_t ledno, uint32_t frame) {
	...
	return rainbow[index] << (16 - inverseBrightness) | rainbow[index + 1] << (8 - inverseBrightness) | rainbow[index + 2] >> inverseBrightness;
	// return rainbow[index + 2];
}
...
uint32_t GetLEDTwinkle(uint32_t ledno, uint32_t frame)
{
	...
	uint32_t ice  = (0x00007f>>inverseBrightness) | ((rsbase>>1)<<(8 - inverseBrightness)) | ((rsbase>>1)<<(16 - inverseBrightness));
	return ice;
}
